...
- ✅ Aba Convênios (config) e KPIs do painel (`ConveniosKpiService`, `ConvenioConfigService`, `ConvenioListagemService`) — `aReceber` e `emRisco` são reais; `glosado`/`recuperado` continuam zerados de propósito nos 4 cards do topo (a agregação sobre `glosa`/`recurso_glosa` ainda não foi escrita ali — os indicadores reais de glosa aparecem na tira dentro da própria aba Glosas, via `GlosaIndicadoresService`)
...
- ✅ **Motor de Auditoria** (`AuditoriaEngineService`) — roda automaticamente na assinatura do prontuário (`ProntuarioEscritaService`, único evento de "atendimento finalizado" acessível no monólito Java hoje — `consulta.status_consulta='Realizada'` é escrito pelo `agenda-service` em Go, fora de alcance). Avalia só 4 dos 9 tipos de regra (`autorizacao_obrigatoria`, `documento_obrigatorio`, `paciente_inelegivel`, `prazo_faturamento_excedido`) — os outros 5 dependem de saber qual `convenio_procedimento` foi realizado, e não existe esse link no schema (só `paciente.id_convenio`, geral). Regras específicas de um procedimento (`regra_auditoria.procedimento != null`) também não avaliam pelo mesmo motivo. Grava em `auditoria_atendimento`/`auditoria_item`; frontend (`AuditoriaLista.jsx`, `AuditoriaDetalhePagina.jsx`, rota `/convenios/auditoria/{id}`) é só leitura — não há ação de "corrigir pendência" nesta fase (as telas de autorização/documento pra isso não existem no frontend ainda). KPI "Em risco" do topo passou a refletir esses dados. Verificado ponta a ponta (backend real + navegador) cobrindo aprovado/atenção/bloqueado.
...
- ⬜ Fix do bug de enum nativo em `RegraAuditoria.java` (`tipo`/`severidade` usam `@Enumerated(EnumType.STRING)`, que quebra escrita num banco onde essas colunas são o enum nativo do Postgres em vez de `VARCHAR`) — não bloqueou o motor de auditoria porque ele só lê `regra_auditoria`, mas trava o CRUD de regras (aba Convênios) num ambiente onde a coluna é o enum de verdade
- ✅ **Aba Lotes** (`LoteSugestaoService`, `LoteEscritaService`, `LoteListagemService`, `LoteDetalheService`) — wizard de 3 passos (sugestão por convênio → revisão dos atendimentos elegíveis, com badge do resultado da Auditoria por item → confirmação e criação), listagem com conciliação (valor enviado/pago/glosado) e alerta de divergência, página de detalhe `/convenios/lotes/{id}` com transições de status validadas (`rascunho→pronto_envio→enviado→processando→{pago,pago_parcial,com_glosas}`). Elegibilidade exclui atendimentos com `auditoria_atendimento.status='bloqueado'`. Verificado ponta a ponta (backend real via curl + navegador via Playwright), inclusive refletindo glosas reais já existentes no banco.

**Nota de ambiente:** a migração `006_recuperacao_glosas.sql` não tinha sido aplicada neste banco de dev até a sessão da Fase A — `recurso_glosa` existia com colunas erradas (criado por `ddl-auto` do Hibernate antes da migração rodar, sem `DEFAULT FALSE` em `evidencias_conferidas`). Corrigido com `ALTER TABLE recurso_glosa ALTER COLUMN evidencias_conferidas SET DEFAULT FALSE` além de rodar a migração. O mesmo tipo de desalinhamento apareceu de novo em `regra_auditoria.tipo`/`severidade` (ver bug de enum acima). Vale conferir se outros ambientes (produção, outros devs) têm o mesmo problema antes de assumir que as migrações 005/006 rodaram limpas em todo lugar.
